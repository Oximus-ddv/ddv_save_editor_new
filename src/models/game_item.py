...
class GameItem(BaseModel):
    ...
    sub_category: Optional[str] = Field(
        None, description="Item sub-category (e.g. from Dict comments)"
    )
    ...
    max_quantity: Optional[int] = Field(
        None, description="Maximum recommended quantity (1 for yellow items)"
    )
...
class PlayerInventoryItem(BaseModel):
    ...
    state: Optional[Any] = Field(
        None, description="Item state (for some special items)"
    )
    inventory_id: Optional[str] = Field(
        None, description="Original inventory/group identifier where this item belongs"
    )
    marker: Optional[str] = Field(
        None, description="Marker for items in ListInventories (e.g. ItemMarker_None)"
    )
    source_type: str = Field(
        "container", description="Source type: 'container' or 'list'"
    )
    raw_data: Dict[str, Any] = Field(
        default_factory=dict,
        description="Original raw data to preserve unmodeled fields",
    )
...
class PetData(BaseModel):
    ...
    name: Optional[str] = Field(
        None,
        description="Deprecated: Original Name field, kept for backward compatibility",
    )
    ...
    granted_inventory_slots: int = Field(
        0, description="Number of inventory slots granted by this pet"
    )
    pending_hangout_rewards: List[Any] = Field(
        default_factory=list, description="Pending hangout rewards"
    )
    raw_data: Dict[str, Any] = Field(
        default_factory=dict,
        description="Original raw data to preserve unmodeled fields",
...
class SaveData(BaseModel):
    ...
    collection_sets: List[Dict[str, Any]] = Field(
        default_factory=list, description="Collection sets data"
    )
...
class ItemCollection(BaseModel):
    ...
    def add_item(self, item: GameItem) -> None:
        ...
        if item.category != self.category:
            raise ValueError(
                f"Item category {item.category} doesn't match collection category {self.category}"
            )
...
class GameDatabase(BaseModel):
    ...
    def get_all_categories(self) -> List[ItemCategory]:
        ...
        return [
            cat for cat, collection in self.collections.items() if len(collection) > 0
        ]
