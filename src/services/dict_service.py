...
class DictDataService:
    ...
    def load_game_database(self, force_reload: bool = False) -> GameDatabase:
        ...
        try:
            ...
            if not self.dict_root.exists():
                logger.error(
                    f"Dict source not found (tried {self.dict_root} and {self.zip_path})"
                )
                return GameDatabase()

            if self.dict_root.is_file():
                self.zip_path = self.dict_root
                return self._load_from_zip(force_reload)
    ...
    def _load_from_zip(self, force_reload: bool = False) -> GameDatabase:
        ...
        try:
            ...
            with zipfile.ZipFile(self.zip_path, "r") as zf:
                ...
                normalized_files = [
                    f.removeprefix("Dict/") for f in all_files if f.startswith("Dict/")
                ]
    ...
    def _guess_category_by_id(self, item_id: int) -> Optional[ItemCategory]:
        ...
        s = str(item_id)
        if s.startswith("10"):
            return ItemCategory.MOTIFS
        if s.startswith("11"):
            return ItemCategory.TOOLS
        if s.startswith("12"):
            return ItemCategory.PETS
        if s.startswith("14"):
            return ItemCategory.MAKEUP
        if s.startswith("16"):
            return ItemCategory.TRIMMING
        if s.startswith("17"):
            return ItemCategory.NPC_SKINS
        if s.startswith("18"):
            return ItemCategory.SCRAMBLECOIN
        if s.startswith("19"):
            return ItemCategory.PHOTO_MODE
        if s.startswith("20"):
            return ItemCategory.HOUSE_SKINS
        if s.startswith("21"):
            return ItemCategory.TOOLS  # Tool skins
        if s.startswith("30"):
            return ItemCategory.MATERIALS
        if s.startswith("31"):
            return ItemCategory.ACTIVITY
        if s.startswith("40"):
            return ItemCategory.FURNITURE
        if s.startswith("5080"):
            return ItemCategory.GLIDERS
        if s.startswith("50"):
            ...
            try:
                ...
                if existing:
                    ...
                    if existing.category == item_category:
                        if not existing.name.startswith("Item ") and len(
                            str(name)
                        ) > len(existing.name):
                    ...
                    # Upgrade priority: allow items to move from 'CLOTHES_OTHER' or 'FURNITURE' to a more specific category
                        ...
                        logger.debug(
                            f"Upgraded item {item_id} from {existing.category} to {item_category}"
                        )
                    continue
        ...
        try:
            for p in sorted(self.dict_root.rglob("*.json")):
                try:
                    stat = p.stat()
                    parts.append(
                        f"{p.relative_to(self.dict_root)}:{int(stat.st_mtime)}:{stat.st_size}"
                    )
                except Exception:
                    continue
        except Exception:
    ...
    def _read_id_name_map_with_metadata(
        self, text: str
        ...
        for line in text.splitlines():
            ...
            if line.startswith("//"):
                ...
                if (
                    cat_name
                    and not cat_name.startswith("Quest Furniture")
                    and not cat_name.startswith("Quest Companion")
                ):
