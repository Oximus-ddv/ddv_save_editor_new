...
class ImageService:
    ...
    def _initialize(self):
        ...
        try:
            ...
            if self.img_cache_dir.exists():
                ...
                logger.info(
                    f"Scanned {len(self._available_images)} images in img_cache"
                )
            else:
    ...
    def get_item_image(
        ...
        logger.warning(
            f"Image not found in img_cache for ID {item_id}. Returning placeholder."
        )
    ...
    def get_image_by_path(
        self, image_path: str, size: Tuple[int, int] = None, for_tkinter: bool = False
    ) -> Optional[Image.Image]:
        ...
        try:
            ...
            if category == ItemCategory.FURNITURE:
                ...
                if resp.status_code == 200 and resp.content:
                    ...
                    image.load()
            elif category.name.startswith("CLOTHES_") or category in {
                ...
                if resp.status_code == 200 and resp.text:
                    ...
                    img_tag = soup.find(
                        "img",
                        {"class": lambda c: c and ("product" in c or "main" in c)},
                    ) or soup.find("img")
    ...
    def cache_image_for_item(
        self, item_id: int, name: str, category: ItemCategory
    ) -> bool:
        ...
        fam = (
            ...
            if category == ItemCategory.FURNITURE
                ...
                or category == ItemCategory.CLOTHES_OTHER
                else "pets" if category == ItemCategory.PETS else category.value
            )
        )
        cache_rel = f"{fam}/{item_id}.png"
        img = self.get_image_by_name_online(
            name,
            category,
    ...
    def _load_image(
        self, image_path: str, size: Tuple[int, int]
    ) -> Optional[Image.Image]:
        ...
        try:
            ...
            if ic_path.exists():
                ...
                logger.warning(
                    f"Image {filename} found in index but missing from disk at {ic_path}"
                )
        except Exception as e:
    ...
    def _create_placeholder_image(
        self, size: Tuple[int, int], text: str = "?"
    ) -> Image.Image:
    ...
    def preload_category_images(
        self, category: ItemCategory, item_ids: list[int]
    ) -> None:
