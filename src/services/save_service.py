...
class SaveFileService:
    ...
    def load_save_file(
        self, file_path: str, decryption_key: Optional[str] = None
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            logger.info(
                f"File encryption status: {'Encrypted' if self.is_encrypted else 'Plain text'}"
            )
            ...
            try:
                ...
                if "Expecting property name enclosed in double quotes" in str(e):
                    return (
                        False,
                        "Save file appears to be corrupted. Try restoring from a backup or using the working copy you mentioned.",
                    )
        ...
        try:
            ...
            for profile_path in base_path.rglob("profile.json"):
                try:
                    ...
                    if not (
                        top_name.startswith("steam") or top_name.startswith("windows")
                    ):
                        continue
            ...
            candidates.sort(
                key=lambda x: (x["modified"], x["folder_mtime"], x["prefer"]),
                reverse=True,
            )
            latest = candidates[0]
    ...
    def auto_load_latest_save(
        self, decryption_key: Optional[str] = None
    ) -> Tuple[bool, str]:
        """
    ...
    def save_file(self, output_path: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            json_data = json.dumps(
                save_dict, separators=(",", ":"), ensure_ascii=False
            )  # Compact format
            ...
            if self.is_encrypted and self.decryption_key:
                ...
                encrypted_data = self._encrypt_data(
                    compressed_data, self.decryption_key
                )
            ...
            if isinstance(pet_data, dict) and "PetItemID" in pet_data:
                pets.append(
                    PetData(
                        ...
                        granted_inventory_slots=pet_data.get(
                            "GrantedInventorySlots", 0
                        ),
                        pending_hangout_rewards=pet_data.get(
    ...
    def _save_data_to_dict(self, save_data: SaveData) -> Dict[str, Any]:
        ...
        if "original_save" in save_data.custom_data:
            ...
            for field in [
                "Xp",
                "NextContainerInventoryID",
                "NextListInventoryID",
                "NextClothingDesignID",
            ]:
        ...
        for pet in save_data.pets:
            ...
            if pet.is_following:
                ...
                total_pet_inventory_slots += (
                    pet.granted_inventory_slots
                )  # Accumulate granted slots from following pets
        ...
        for inv_id, items in inventory_groups.items():
            ...
            if not container:
                ...
                size_for_new_container = (
                    expected_backpack_size if inv_id == "0" else len(items) + 10
                )
                container = {
                    "ID": int(inv_id) if inv_id.isdigit() else 0,
                    "Size": size_for_new_container,
                    "Inventory": [],
                }
                if inv_id == "0":
                    container["ExtraBagSpace"] = total_pet_inventory_slots
            ...
            target_size = (
                expected_backpack_size
                if inv_id == "0"
                else container.get("Size", len(new_inv_list))
            )
