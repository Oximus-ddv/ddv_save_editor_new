...
from PyQt6.QtWidgets import (
    ...
    QTreeWidget, QTreeWidgetItem, QPushButton, QHeaderView, QCheckBox, QMessageBox
)
...
class CollectionSetEditorFrame(QWidget):
    ...
    def __init__(self, parent, save_service: SaveFileService, dict_service: DictDataService, image_service: ImageService):
        ...
        self.setup_ui()
        self.hover_behavior = HoverPreviewBehavior(self.collection_set_tree, self.image_service, self._get_category_for_item)

    def _get_category_for_item(self, item_id: int, item: QTreeWidgetItem) -> Optional[ItemCategory]:
    ...
    def setup_ui(self):
        ...
        self.collection_set_tree.setHeaderLabels(["Collection / Item", "Item ID", "Collected"])
        self.collection_set_tree.header().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.collection_set_tree.header().setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        self.collection_set_tree.header().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
    ...
    def refresh_collection_set_tree(self):
        ...
        for collection_set in self.save_data.collection_sets:
            ...
            for group_data in collection_set.get('GroupData', []):
                ...
                group_item.setText(0, f"{display_name} (ID: {collection_set.get('CollectionDefinitionID', 'N/A')})")
                ...
                if 'GroupsCollectionItems' in group_data:
                    ...
                    for item_id, is_collected in group_data['GroupsCollectionItems'].items():
                    ...
                    for item_name, item_id, is_collected in items_to_sort:
                        ...
                        checkbox.toggled.connect(
                            lambda checked, item_id=item_id, group_data=group_data: 
                            self._on_checkbox_toggled(checked, item_id, group_data)
                        )
    ...
    def _on_checkbox_toggled(self, is_checked: bool, item_id: str, group_data: dict):
        ...
        if 'GroupsCollectionItems' in group_data and item_id in group_data['GroupsCollectionItems']:
            group_data['GroupsCollectionItems'][item_id] = is_checked
            if is_checked:
                if self._add_item_to_inventory(int(item_id)):
                    QMessageBox.information(self, "Item Added", f"Item {item_id} has been added to your inventory.")
    ...
    def _add_item_to_inventory(self, item_id: int) -> bool:
        ...
        print(f"DEBUG: self.game_database is {'present' if self.game_database else 'None'}")

        if not self.save_data or not self.game_database:
            print(f"DEBUG: Skipping adding item {item_id}: No save data or game database available.")
            logger.debug(f"Skipping adding item {item_id}: No save data or game database available.")
            return False

        existing_item_ids = {item.item_id for item in self.save_data.inventory_items}
        if item_id in existing_item_ids:
            print(f"DEBUG: Item {item_id} already exists in inventory. Skipping addition.")
            logger.info(f"Item {item_id} already exists in inventory. Skipping addition.")
            return False
        ...
        if game_item:
            ...
            logger.info(f"Successfully added item {item_id} ({game_item.name}) to inventory {inventory_id}.")
            return True
        else:
            print(f"DEBUG: Could not find game item with ID {item_id} in the database.")
            logger.warning(f"Could not find game item with ID {item_id} in the database to add to inventory.")
            return False
    ...
    def unlock_and_add_all_items(self):
        ...
        if not self.save_data or not self.save_data.collection_sets or not self.game_database:
            return
        ...
        self.refresh_collection_set_tree()
        QMessageBox.information(self, "Unlock All", f"All collection items unlocked. {items_added_count} new items added to your inventory.")
