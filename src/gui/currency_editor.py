...
class CurrencyEditorFrame(QWidget):
    ...
    def __init__(
        self, parent, save_service: SaveFileService, danger_zone_enabled: bool = False
    ):
    ...
    def setup_currencies_section(self, parent_layout):
        ...
        currencies = [
            ...
            (
                "Dreamlight:",
                self.dreamlight_spinbox,
                "Used for unlocking areas and features",
            ),
            ...
            (
                "StoryBook Magic:",
                self.story_book_magic_spinbox,
                "Currency for Storybook content",
            ),
        ]

        moonstone_currency = (
            "Moonstones:",
            self.moonstones_spinbox,
            "Premium currency (WARNING: May not work)",
        )
        ...
        for i, (label_text, spinbox, tooltip) in enumerate(currencies):
            ...
            max_btn.clicked.connect(
                lambda checked, sb=spinbox: self.set_max_currency(sb)
            )
