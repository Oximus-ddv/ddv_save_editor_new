...
class CollectionEditorFrame(QWidget):
    ...
    def __init__(self, parent, save_service: SaveFileService, game_database: GameDatabase):
    ...
    def setup_ui(self):
        ...
        self.collection_tree.setHeaderLabels(["Collection / Item", "Collected", "Locked"])
        self.collection_tree.header().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.collection_tree.header().setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        self.collection_tree.header().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
    ...
    def complete_all_collections(self):
        ...
        all_item_ids = {item.id for col in self.game_database.collections.values() for item in col.items.values()}
        ...
        for set_data in self.save_data.collection_sets:
            if 'Entries' in set_data:
                ...
                for item_id in missing_item_ids:
                    ...
                    set_data['Entries'].append({'ItemId': item_id, 'IsCollected': True, 'IsLocked': False})
