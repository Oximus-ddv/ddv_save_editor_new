...
class ItemEditorFrame(QWidget):
    ...
    def setup_ui(self):
        ...
        back_btn.clicked.connect(self.back_button_clicked.emit)
        back_btn.clicked.connect(self.hide_previews)
        ...
        self.dict_tree.setAlternatingRowColors(True)
        self.dict_tree.setHeaderLabels(["ID", "Name"])
        self.dict_tree.header().setSectionResizeMode(
        ...
        self.dict_tree.header().sectionClicked.connect(
            lambda col: self._sort_tree(self.dict_tree, col)
        )
        # Enable multi-selection
        from PyQt6.QtWidgets import QAbstractItemView

        self.dict_tree.setSelectionMode(
        ...
        self.inv_tree.setAlternatingRowColors(True)
        self.inv_tree.setHeaderLabels(["ID", "Name", "Amount"])
        self.inv_tree.header().setSectionResizeMode(
            0, QHeaderView.ResizeMode.Interactive
        )
        self.inv_tree.header().setSectionResizeMode(
            1, QHeaderView.ResizeMode.Interactive
        ...
        # Enable multi-selection
        self.inv_tree.setSelectionMode(
            QAbstractItemView.SelectionMode.ExtendedSelection
        )
        self.inv_tree.itemDoubleClicked.connect(self.edit_inventory_amount)
        self.inv_tree.setSortingEnabled(True)
        self.inv_tree.header().sectionClicked.connect(
            lambda col: self._sort_tree(self.inv_tree, col)
        )
        ...
        # Conditionally add "Edit Amount" button
        if self.category != ItemCategory.PETS:
        ...
        self.dict_hover = HoverPreviewBehavior(
            self.dict_tree, self.image_service, lambda id, item: self.category
        )
        self.inv_hover = HoverPreviewBehavior(
            self.inv_tree, self.image_service, lambda id, item: self.category
        )

    def hide_previews(self):
        """Hides any active hover previews."""
        self.dict_hover.hide_preview()
        self.inv_hover.hide_preview()
    ...
    def populate_dictionary(self, saved_item_ids: set):
        ...
        for item in sorted_collection:
            ...
            if self.category in [
                ...
                if is_house_wallpaper:
                    passes_house_filter = item_id_str.startswith(
                        "20"
                    ) or item_id_str.startswith("160")
            ...
            # Furniture category special filtering: exclude items starting with '30' or '31'
            elif self.category == ItemCategory.FURNITURE and (
                item_id_str.startswith("30") or item_id_str.startswith("31")
            ):
                continue  # Skip '30' and '31' items from Furniture category
        ...
        self.dict_tree.resizeColumnToContents(0)
        self.dict_tree.resizeColumnToContents(1)
    ...
    def refresh_inventory_tree(self):
        ...
        if self._is_pet_category():
            ...
            items_to_display_count = len(self.save_data.pets)

        # Handle Tools specially
        elif self._is_tool_category():
            tools = (
                self.save_data.custom_data.get("original_save", {})
                .get("Player", {})
                .get("Tools", [])
            )
            for tool in tools:
                self._add_tool_tree_item(tool)
            items_to_display_count = len(tools)

        else:
            items_to_display = []
            for inv_item in self.save_data.inventory_items:
                # For Furniture tab, skip items with IDs starting with '30' or '31'
                if self.category == ItemCategory.FURNITURE and (
                    str(inv_item.item_id).startswith("30")
                    or str(inv_item.item_id).startswith("31")
                ):
                    continue
                ...
                elif self._item_matches_category(
                    str(inv_item.item_id)
                ):  # Fallback for items not in collection but match category pattern
                    items_to_display.append(inv_item)

            for item in items_to_display:
                self._add_inv_tree_item(item)
            items_to_display_count = len(items_to_display)

        logger.info(
            f"Category {self.category.name}: {items_to_display_count} items displayed in inventory tree"
        )
        for i in range(self.inv_tree.columnCount()):
            self.inv_tree.resizeColumnToContents(i)
    ...
    def _item_matches_category(self, item_id_str: str) -> bool:
        ...
        if self.category == ItemCategory.FURNITURE:
            return (
                item_id_str.startswith("30")
                or item_id_str.startswith("31")
                or item_id_str.startswith("40")
                or item_id_str.startswith("41")
            )
        ...
        if self.category == ItemCategory.MATERIALS:
            return item_id_str.startswith("10") and not item_id_str.startswith(
                "100"
            )  # Avoid Motifs conflict
    ...
    def _add_inv_tree_item(
        self, inv_item: PlayerInventoryItem, show_as_unknown: bool = False
    ):
    ...
    def add_selected_item(self):
        ...
        for item in selected_items:
            # If item is a group (has children), skip it.
            ...
            if self.category in [
                ...
                if not str(game_item.id).startswith("20"):
                    QMessageBox.warning(
                        self,
                        "Cannot Add",
                        f"Item '{game_item.name}' (ID: {game_item.id}) cannot be added to Houses category as it does not start with '20'.",
                    )
            ...
            if self.category in [
                ...
                passes_house_filter = False
                if is_house_wallpaper:
                    passes_house_filter = game_item_id_str.startswith(
                        "20"
                    ) or game_item_id_str.startswith("160")
                else:  # Other house types (SKINS, FLOORS, NPC_HOUSES)
                ...
                game_item_id_str.startswith("30") or game_item_id_str.startswith("31")
            ):
                should_add_item = False
                reason = f"Item ID {game_item_id_str} starts with '30' or '31' and cannot be added to the Furniture category."

            if not should_add_item:
                QMessageBox.warning(
                    self,
                    "Cannot Add",
                    f"Cannot add item '{game_item.name}' (ID: {game_item.id}). Reason: {reason}",
                )
                continue  # Skip adding this item
            # --- END NEW CHECK ---
            ...
            if self._is_pet_category():
                ...
                player_data = self.save_data.custom_data["original_save"].setdefault(
                    "Player", {}
                )
                ...
                if add_specific_tool(
                    self.save_data.custom_data["original_save"], game_item.id
                ):
                    added_count += 1
            else:
                if add_item_from_editor(
                    self.save_data.custom_data["original_save"],
                    game_item.id,
                    self.category.name,
                ):
                    added_count += 1
        ...
        if added_count > 0:
            ...
            if self.category == ItemCategory.TOOLS:
                message = (
                    f"{added_count} new tool(s) have been added to your tools list."
                )
    ...
    def remove_selected_item(self):
        ...
        if count == 1:
            ...
            item_id = getattr(
                inv_item, "item_id", getattr(inv_item, "pet_item_id", "Unknown")
            )
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            ...
            for tree_item in selected_items:
                ...
                item_id_for_log = getattr(
                    item_data, "item_id", getattr(item_data, "pet_item_id", "Unknown")
                )
                removed_items_log.append(
                    f"ID: {item_id_for_log}, Name: {tree_item.text(1)}"
                )
            ...
            if removed_count > 0:
                logger.info(
                    f"User removed {removed_count} items in category {self.category.name}: {removed_items_log}"
                )
            self.data_changed.emit()
            self.refresh_inventory_tree()
    ...
    def edit_inventory_amount(self):
        ...
        if not editable_items:
            if any(
                hasattr(tree_item.data(0, Qt.ItemDataRole.UserRole), "pet_item_id")
                for tree_item in selected_items
            ):
                QMessageBox.information(
                    self, "Info", "Pets/Companions do not have an amount."
                )
            return
        ...
        prompt = (
            ...
            else f"Enter amount for item {getattr(editable_items[0][1], 'item_id', '?')}:"
        )
    ...
    def edit_pet_details(self):
        """Open the pet editor dialog for the selected pet(s)"""
        selected_items = self.inv_tree.selectedItems()
        if not selected_items:
            return
        ...
        pets_data = []
        for item in selected_items:
            pet_data = item.data(0, Qt.ItemDataRole.UserRole)
            if hasattr(pet_data, "pet_item_id"):
        ...
        if len(pets_data) == 1:
            pet_to_edit = pets_data[0]
            game_item = self.collection.get_item(pet_to_edit.pet_item_id)
            dialog = PetEditorDialog(self, pet_to_edit, game_item, self.image_service)
            if dialog.exec():
                # For single edit, we can just update the tree item text for name changes
                # But a full refresh is safer and handles other changes too
                self.refresh_inventory_tree()
                self.data_changed.emit()
        else:
            # Multi-edit
            from .multi_pet_editor_dialog import MultiPetEditorDialog

            dialog = MultiPetEditorDialog(self, len(pets_data))
            if dialog.exec():
                new_values = dialog.get_values()
                level = new_values["level"]
                xp = new_values["xp"]
                slots = new_values["slots"]

                for pet in pets_data:
                    if level != -1:
                        pet.friendship_level = level
                    if xp != -1:
                        pet.xp = xp
                    if slots != -1:
                        pet.granted_inventory_slots = slots
        ...
        for game_item in self.collection:
            if game_item.id not in existing_ids:
                ...
                self.save_data.inventory_items.append(
                    PlayerInventoryItem(
                        item_id=game_item.id, amount=1, inventory_id=inv_id
                    )
                )
                count += 1
